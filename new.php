...
$sql = "SELECT COUNT(TmpId) FROM $Database[TablePrefix]ChatLiveUsers WHERE IsGuest = '1'";
$result = mysql_query ($sql) or Dienice('&e=19', $sql);
...
if ($n_gen == '1')
	{$OtherNamesExt = '0';
	$sql = "SELECT Name FROM $Database[TablePrefix]ChatLiveUsers WHERE Name LIKE \"$Name%\" ORDER BY Name DESC LIMIT 1 ";
	$result = mysql_query ($sql) or Dienice('&e=19', $sql);
...
if ($Item[Low] > 1) {$Tmp[0] = $Item[Low] - 1;$sql = "INSERT INTO $Database[TablePrefix]ChatLiveUsers SET TmpId = '$Tmp[0]',Pass='$User[Pass]',RoomId='0',LastCheckIn='$now',Name='$Name',IsGuest='1'";}
else {$Tmp[0] = $Item[High] + 1;$sql = "INSERT INTO $Database[TablePrefix]ChatLiveUsers SET TmpId = '$Tmp[0]',Pass='$User[Pass]',RoomId='0',LastCheckIn='$now',Name='$Name',IsGuest='1'";}
$result = mysql_query ($sql) or Dienice('&e=19', $sql);
...
if (mysql_num_rows($result) == 0) 	{$sql = "INSERT INTO $Database[TablePrefix]ChatDailyStats SET Year='$Year',Month='$Month',Day='$Day',Guests='1',MaxGuests='$Item[MaxGuests]',MaxUsers='$Item[MaxUsers]'";}
else 					{$sql = "UPDATE $Database[TablePrefix]ChatDailyStats SET Guests=Guests+1,MaxGuests='$Item[MaxGuests]',MaxUsers='$Item[MaxUsers]' WHERE Year='$Year' AND Month='$Month' AND Day='$Day'";}
$result = mysql_query ($sql) or Dienice('&e=19', $sql);
